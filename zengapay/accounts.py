...
class Accounts(ZengaPayAPI):
    ...
    def get_account_statement(self, url=None, limit=0, **kwargs):
        ...
        if "designation" in kwargs and kwargs.get("designation").upper() in designations:
            url += f"&designation={kwargs.get('designation')}"
        if "currency_code" in kwargs and kwargs.get("currency_code").upper() in currency_codes:
